...
%condition 3: data for free viewing analysis, without fixations in center 
saveFlag=0;
...
elseif condition == 3
    ...
    Paths.ResultsStructsPath=[Paths.FoldersPath,'\ResultsStructs\FreeViewingRemoveCenterBias'];
end
cd(Paths.EyeTrackingAnalysisFolder)
...
for ii=1:length(EXPDATA_ALL) %subjects
    ...
    if condition == 3
        ...
        EXPDATA_ALL{ii}=removeSpecificImages(EXPDATA_ALL{ii});
    end
