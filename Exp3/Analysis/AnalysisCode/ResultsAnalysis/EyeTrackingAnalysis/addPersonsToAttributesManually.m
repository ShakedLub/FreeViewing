function addPersonsToAttributesManually(Paths)
...
%check attrs exists and empty
if isfile('AttrsCombined.mat')
    error('AttrsCombined.mat exists')
end
...
for ii=1:length(ImNames) %images
    numPersonObj=0;
    numPersonObjFound=0;
    numPersonObjAdded=0;
    found_person_objects_ind_person=[];
    found_person_objects_ind_OSIE=[];
    combiningData(ii).ImName=ImNames{ii};
    
    indImAttr=find(strcmp(ImNames{ii},ImgNamesAttr));
            ...
            if (attrsP{indImAttr}.objs{op}.features(indPerson)==1) %if this is a person object
                ...
                for aa=1:length(OrderOfOverlappingObj) %objects in attrs
                    ...
                    if (OverlapSize/PersonSize)*100 >= minimalOverlapPercent    
                        ...
                        subplot(2,3,4)
                        I=imread([Paths.ImagesFolder,'\',ImNames{ii}]);
                        imshow(I)
                        title(ImNames{ii})
    ...
    combiningData(ii).numPersonObj=numPersonObj;
    combiningData(ii).numPersonObjFound=numPersonObjFound;
    combiningData(ii).numPersonObjAdded=numPersonObjAdded;
    combiningData(ii).found_person_objects_ind_person_dataset=found_person_objects_ind_person;
    combiningData(ii).found_person_objects_ind_OSIE_dataset=found_person_objects_ind_OSIE;
