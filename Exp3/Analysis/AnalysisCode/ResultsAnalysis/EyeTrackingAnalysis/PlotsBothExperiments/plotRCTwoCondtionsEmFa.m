function plotRCTwoCondtionsEmFa(dataP,prc,measure,rows,columns,plotNum,Paths,FlagAstU,FlagAstC,xexponent)
%%rain cloud plot
%addpath cd cbrewer
...
for ii=1:2 %two experiments
    ...
    yt = yt(yt>=0);
    yt = [yt(1),yedge(2)];
    if round(yt(2),1)>yt(2) %round by flooring, with 1 number after the dot
        yt(2)=yt(2)-0.05;
    end   
    yt=round(yt,1);
    yticks(yt)
