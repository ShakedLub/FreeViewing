...
elseif condition == 2
    Paths.ResultsStructsPath=[Paths.FoldersPath,'\ResultsStructs\Pilot',Param.PILOT_NUM,'_LowLevelType1'];
elseif condition == 3
...
Param.pixels_per_vdegree=EXPDATA_ALL{1,1}.info.lab_setup.pixels_per_vdegree;
...
Param.window_size_cm=[EXPDATA_ALL{1,1}.info.lab_setup.screen_width_in_cm,EXPDATA_ALL{1,1}.info.lab_setup.screen_height_in_cm];
...
%% arrange fixations per subject
%image trials data arrange per subject
...
if any([TrialsRemoved.numTrialsSummaryFinal.ImageTrialsU]<Param.MinTrials | [TrialsRemoved.numTrialsSummaryFinal.ImageTrialsC]<Param.MinTrials)
    error('Some subjects have less than 10 image unconscious trials or conscious trials')
end
...
if condition ~= 2 %Do not do these calculations in Low level type 1 analysis
    ...
    AttributesResults=classifyFixationsAttributes(ObjectsResults,FixationsPerImageProcessed,Param,attrs);
    if Param.shuffledDataFlag
