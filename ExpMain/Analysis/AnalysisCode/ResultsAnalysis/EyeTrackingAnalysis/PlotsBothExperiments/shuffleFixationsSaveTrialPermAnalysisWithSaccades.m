function fixations=shuffleFixationsSaveTrialPermAnalysisWithSaccades(fixations,Fixations_PerSubject,subjNumber,NRepetition)
    ...
    for ii=1:size(fixations,2) %images
        for kk=1:size(fixations(ii).condition,2) %condition
            for jj=1:size(fixations(ii).condition(kk).subject,2) %subjects
                ...
                fixations(ii).shuffled(nn).condition(kk).subject(jj).fix_onsets_final=Fixations_PerSubject(indSubj).condition(kk).trial(RandomTrialNum).processed.fix_onsets_final;
                fixations(ii).shuffled(nn).condition(kk).subject(jj).sacc_onsets_final=Fixations_PerSubject(indSubj).condition(kk).trial(RandomTrialNum).processed.sacc_onsets_final;
                fixations(ii).shuffled(nn).condition(kk).subject(jj).sacc_duration_final=Fixations_PerSubject(indSubj).condition(kk).trial(RandomTrialNum).processed.sacc_duration_final;
