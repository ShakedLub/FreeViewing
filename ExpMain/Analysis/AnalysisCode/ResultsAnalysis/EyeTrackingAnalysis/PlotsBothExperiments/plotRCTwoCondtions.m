function plotRCTwoCondtions(dataP,prc,titleText,measure,rows,columns,plotNum,Paths,FlagAstU,FlagAstC,xexponent,colors)
%%rain cloud plot
wd=cd(Paths.RainCloudPlot);

cl1=colors(1,:);%cb(5,:);
clline1=colors(2,:); %[0 0.4470 0.7410];

cl2=colors(3,:);%cb(10,:);
clline2=colors(4,:); %[0.4940 0.1840 0.5560];
...
for ii=1:2 %two experiments
    ...
    if ~isempty(FlagAstU)
        if ( (prc == 5 && realVal1 < alpha1(ii)) || (prc == 95 && realVal1 > alpha1(ii)) ) &&  FlagAstU(ii)==1 %significant
            plot(realVal1,lim(4),'*','MarkerSize',10,'Color',clline1);
        end
    else
    ...
    if ~isempty(FlagAstC)
        if ( (prc == 5 && realVal2 < alpha2(ii)) || (prc == 95 && realVal2 > alpha2(ii)) ) &&  FlagAstC(ii)==1 %significant
            plot(realVal2,lim(4),'*','MarkerSize',10,'Color',clline2);
        end
    else
...
for ii=1:2 %two experiments
    ...
    if ii==2 && ismember(plotNum(ii),[columns+2,columns+4,columns*3+2,columns*3+4])
        xlabel(xlabelText)
    end
