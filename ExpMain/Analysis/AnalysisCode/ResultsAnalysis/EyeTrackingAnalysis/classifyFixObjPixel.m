function count=classifyFixObjPixel(count,centerH,centerW,dur,imsize,objs,Param,Paths,ImName)
%create circle mask
plotFlag=0;
imageSizeH=imsize(1);
imageSizeW=imsize(2);
...
for bb=1:size(objs,1) %objects
    if objs{bb}.bw(centerH,centerW)==1
...
elseif length(OptionalObj)==1 %1 object
    ...
    if plotFlag
        ...
        im=zeros(imsize);
        im(centerH,centerW)=1;
        imshow(im,[])
        title(['Fixation, classification:',num2str(ObjClass)])
