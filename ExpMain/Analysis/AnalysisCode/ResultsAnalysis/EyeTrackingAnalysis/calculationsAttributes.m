function [Results,Summary]=calculationsAttributes(Results,Param,attrNames,condition)
...
Results.effectSizeFPPAtt=nan(size(Results.image(1).condition,2),length(attrNames));
...
Results.effectSizeFDPPAtt=nan(size(Results.image(1).condition,2),length(attrNames));
...
for kk=1:size(Results.image(1).condition,2) %condition
    ...
    Results.effectSizeFPPAtt(kk,Param.EFAtt)=(Results.FixPerPixAtt(kk,Param.EFAtt)-mean(Results.condition(kk).FixPerPixAttShuffled(:,Param.EFAtt)))/std(Results.condition(kk).FixPerPixAttShuffled(:,Param.EFAtt));
    ...
    Results.effectSizeFDPPAtt(kk,Param.EFAtt)=(Results.FixDurPerPixAtt(kk,Param.EFAtt)-mean(Results.condition(kk).FixDurPerPixAttShuffled(:,Param.EFAtt)))/std(Results.condition(kk).FixDurPerPixAttShuffled(:,Param.EFAtt));
end
...
Summary.effectSizeFPPAtt=Results.effectSizeFPPAtt;
...
Summary.effectSizeFDPPAtt=Results.effectSizeFDPPAtt;
