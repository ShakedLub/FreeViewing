function count=classifyFixObj(count,centerH,centerW,dur,imsize,objs,Param,Paths,ImName)
%create circle mask
plotFlag=0;
imageSizeH=imsize(1);
imageSizeW=imsize(2);
...
if ~isnan(ObjClass)
    ...
    numPixCircle=sum(sum(circleMask));
    
    %number of pixels in overlap between fix and object
    numPixOverlapFixObj=sum(objs{ObjClass}.bw(circleMask));
    
    %number of pixels in object
    numPixObj=sum(sum(objs{ObjClass}.bw));
    
    %calculate overlap between object and circle
    percentOverlapObjectCircle=numPixOverlapFixObj/numPixCircle;
    if numPixObj<numPixCircle
        isObjectSmallerThanCircle=1;
    else
        isObjectSmallerThanCircle=0;
    end
else
    percentOverlapObjectCircle=NaN;
    isObjectSmallerThanCircle=NaN;
end
...
count.percentOverlapObjectCircle=[count.percentOverlapObjectCircle,percentOverlapObjectCircle];
count.isObjectSmallerThanCircle=[count.isObjectSmallerThanCircle,isObjectSmallerThanCircle];
end