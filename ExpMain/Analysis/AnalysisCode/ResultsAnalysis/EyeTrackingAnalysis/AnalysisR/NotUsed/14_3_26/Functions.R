...
fixCountMixedModel = function(data,results) {
  ############### fixation count ###############
  #create dataframe for analysis
  vec=c(1,4,10,6)
  dataA=data[,vec]
  names(dataA)[4]='parameter'
  
  #clean missing values
  dataA <- na.omit(dataA) 
  
  m<-lmer(parameter~1+ROI_size +(1+ROI_size|participant)+(1+ROI_size|image_name),data=dataA,control=lmerControl("bobyqa"))
  a=anova(m)
  
  results["param_name"]="fixation_count"
  results["p_ROI_size"]=a$`Pr(>F)`[1]
