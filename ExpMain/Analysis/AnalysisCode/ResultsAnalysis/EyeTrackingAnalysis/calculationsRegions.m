...
for kk=1:size(Results.image(1).condition,2) %condition
    ...
    for gg=1:length(Summary.GroupNames)
        ...
        Summary.condition(kk).effectSizeFPP(gg)=(Summary.condition(kk).FixPerPix(gg)-mean(Summary.condition(kk).FixPerPixShuffled(:,gg)))/std(Summary.condition(kk).FixPerPixShuffled(:,gg));
        Summary.condition(kk).effectSizeFDPP(gg)=(Summary.condition(kk).FixDurPerPix(gg)-mean(Summary.condition(kk).FixDurPerPixShuffled(:,gg)))/std(Summary.condition(kk).FixDurPerPixShuffled(:,gg));
    end
end
end