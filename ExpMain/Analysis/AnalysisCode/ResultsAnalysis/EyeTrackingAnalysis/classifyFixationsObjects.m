...
for ii=1:size(fixations,2) %images
    ...
    for kk=1:size(fixations(ii).condition,2) %conditions
        for jj=1:size(fixations(ii).condition(kk).subject,2) %subjects
            ...
            count.fixDurBg=[];
            if Param.UseCircleAroundFix == 3
                count.percentOverlapObjectCircle=[];
                count.isObjectSmallerThanCircle=[];
            end
            ...
            for aa=1:length(fix_h) %fixations
                ...
                if Param.UseCircleAroundFix == 1
                    count=classifyFixObj(count,centerH,centerW,dur,imsize,objs,Param,Paths,Results.image(ii).imageName);
                elseif Param.UseCircleAroundFix == 0
                    count=classifyFixObjPixel(count,centerH,centerW,dur,imsize,objs,Param,Paths,Results.image(ii).imageName);
                elseif Param.UseCircleAroundFix == 2
                    count=classifyFixObjStringentRule(count,centerH,centerW,dur,imsize,objs,Param,Paths,Results.image(ii).imageName);
                elseif Param.UseCircleAroundFix == 3
                    count=classifyFixObj_countOverlap(count,centerH,centerW,dur,imsize,objs,Param,Paths,Results.image(ii).imageName);
                end
            end
            ...
            if Param.UseCircleAroundFix == 3
                Results.image(ii).condition(kk).subject(jj).percentOverlapObjectCircle=count.percentOverlapObjectCircle;
                Results.image(ii).condition(kk).subject(jj).isObjectSmallerThanCircle=count.isObjectSmallerThanCircle;
            end
        end
