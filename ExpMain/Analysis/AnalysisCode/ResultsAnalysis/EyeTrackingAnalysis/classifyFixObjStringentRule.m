function count=classifyFixObjStringentRule(count,centerH,centerW,dur,imsize,objs,Param,Paths,ImName)
%create circle mask
plotFlag=0;
imageSizeH=imsize(1);
imageSizeW=imsize(2);
...
%check which objects the fixation overlap and check if the object fills above 50% of the circle
%If the region is smaller than the circle, check that aboce 50% of the region is inside the circle
OptionalObj=[];
for bb=1:size(objs,1) %objects
    if sum(objs{bb}.bw(circleMask))>=1
        ...
        numPixOverlapFixObj=sum(objs{bb}.bw(circleMask));
        
        %number of pixels in object
        numPixObj=sum(sum(objs{bb}.bw));
        
        if (numPixOverlapFixObj/numPixCircle) > Param.OverlapRule
            OptionalObj=[OptionalObj,bb];
        elseif numPixObj<numPixCircle && (numPixOverlapFixObj/numPixObj) > Param.OverlapRule
            OptionalObj=[OptionalObj,bb];
        end
    end
end
