function [Results,Summary]=calculationsObjects(Results,Param,condition)
...
for kk=1:size(Results.image(1).condition,2) %condition
    ...
    Results.effectSizeFPPObj(kk)=(Results.FixPerPixObj(kk)-mean(Results.FixPerPixObjShuffled(:,kk)))/std(Results.FixPerPixObjShuffled(:,kk));
    ...
    Results.effectSizeFPPBg(kk)=(Results.FixPerPixBg(kk)-mean(Results.FixPerPixBgShuffled(:,kk)))/std(Results.FixPerPixBgShuffled(:,kk));
    ...
    Results.effectSizeFDPPObj(kk)=(Results.FixDurPerPixObj(kk)-mean(Results.FixDurPerPixObjShuffled(:,kk)))/std(Results.FixDurPerPixObjShuffled(:,kk));
    ...
    Results.effectSizeFDPPBg(kk)=(Results.FixDurPerPixBg(kk)-mean(Results.FixDurPerPixBgShuffled(:,kk)))/std(Results.FixDurPerPixBgShuffled(:,kk));
end
...
Summary.effectSizeFPPObj=Results.effectSizeFPPObj;
...
Summary.effectSizeFPPBg=Results.effectSizeFPPBg;
...
Summary.effectSizeFDPPObj=Results.effectSizeFDPPObj;
...
Summary.effectSizeFDPPBg=Results.effectSizeFDPPBg;
...
if condition ~= 10 && condition ~= 13 && condition ~= 19
    %% plot
    figure
    rows=2;
    columns=2;
