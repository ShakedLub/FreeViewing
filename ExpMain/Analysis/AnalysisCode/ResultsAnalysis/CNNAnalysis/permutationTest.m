function [pval,numExtremeObs,numAllObs,effectSize,DataRand]=permutationTest(Results,FixMaps,Activations,Param)
...
for cc=1:size(FixMaps.condition,2) %conditions
    for ll=1:size(Activations.layer,2) %layer
        ...
        effectSize(cc,ll)=(mean(DataRand(cc,ll,:))-Results.Rho(cc,ll))/std(DataRand(cc,ll,:));
    end
end
end