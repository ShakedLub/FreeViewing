...
if isfield(Results,'stdErr')
    minErr=Results.CIlength(:,:,1)';
    maxErr=Results.CIlength(:,:,2)';
    bh=ErrorBarOfCIOnGroupedBars(Results.Rho',Results.meanErr',minErr,maxErr);
else
    bh=bar(Results.Rho');
end
...
if isfield(Results,'noise')
    line([lim(1),lim(2)],[Results.noise.noise(1),Results.noise.noise(1)],'Color',[0 0.4470 0.7410],'LineWidth',2) %U condition noise floor
    line([lim(1),lim(2)],[Results.noise.noise(2),Results.noise.noise(2)],'Color',[0.4940 0.1840 0.5560],'LineWidth',2) %C condition noise floor
end
hold off
