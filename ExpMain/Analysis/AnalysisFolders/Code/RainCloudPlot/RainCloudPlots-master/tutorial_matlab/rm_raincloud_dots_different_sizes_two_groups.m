...
function h = rm_raincloud(data, dotSize, labels, colours, plot_top_to_bottom, density_type, bandwidth)
...
spacing     = 0.4 * mean(mean(cellfun(@max, ks)));
%ks_offsets  = [0:n_plots_per_series-1] .* spacing;
ks_offsets  = [0:n_series-1] .* spacing; %offset is between series (and not between plots)
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        ...
        % scatter rainclouds (first group)
        scatter(data{i, j}(labels{i, j}==1), -jit{i, j}(labels{i, j}==1) + ks_offsets(j),'o', 'MarkerFaceColor', colours(j, :), 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.5, 'SizeData', dotSize{i,j}(labels{i, j}==1));
        
        % scatter rainclouds (second group)
        scatter(data{i, j}(labels{i, j}==2), -jit{i, j}(labels{i, j}==2) + ks_offsets(j),'s','MarkerFaceColor', colours(j, :), 'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.5, 'SizeData', dotSize{i,j}(labels{i, j}==2));
    end
end
