...
gabor2D= background_color*(gabor+1);
alpha_mat = ones(imSize, imSize);
alpha_mat(abs(gabor2D - 0.5) < 0.01 & gauss < 0.05) = 0;
gabor4D = NaN(imSize, imSize, 4);
gabor4D(:, :, 1:3) = repmat(gabor2D, 1, 1, 3);
gabor4D(:, :, 4) = alpha_mat;

gabor_texture= Screen('MakeTexture', window, gabor4D); 
